...
class Alphas(object):
    ...
    def alpha023(self):
        ...
        alpha = -1 * delta(self.high, 2).fillna(value=0)
    ...
    def alpha031(self):
        ...
        decay = decay_linear((-1 * rank(rank(delta(self.close, 10)))), 10) 
    ...
    def alpha039(self):     # TODO apply this fix to all other decay_linear usages
        adv20 = sma(self.volume, 20)
        decay = decay_linear((self.volume / adv20), 9)
        decay = decay.squeeze()
        return ((-1 * rank(delta(self.close, 7) * (1 - rank(decay)))) *
