...
void loadCurve(Market& mkt, const string& fileName, const string& curveName)
{
...
int main()
{
	...
	loadCurve(*mkt, "usd_curve.txt", "USD-SOFR");
	loadCurve(*mkt, "sgd_curve.txt", "SGD-SORA");
	mkt->Print();

	auto usdCurve = mkt->getCurve("USD-SOFR");
	double rate = usdCurve->getRate(Date(2026, 1, 1));
	...
	auto pricer = new CRRBinomialTreePricer(50);
