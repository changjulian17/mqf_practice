...
public:
	CurveDecorator(const Market& mkt, const MarketShock& curveShock) : thisMarketUp(mkt), thisMarketDown(mkt)  // this is a copy operation of a shared pointer
																												// need to use shared_ptr for deep copy
	{
...
private:
// creating two markets for shock up and down
	Market thisMarketUp;  // market with shock up
	Market thisMarketDown;	 // market with shock down
};
...
public:
	...
	RiskEngine(const Market& market, double curve_shock, double vol_shock, double price_shock) {
		...
		// usdCurveShock.market_id = "USD-SOFT";
		// usdCurveShock.shock = make_pair(Date(), .0001);
