	...
	if (singleThread) {
		if (riskType == "dv01") {
			for (auto& kv : curveShocks) {
				...
				double dv01 = (pv_up - pv_down) / 2.0;  // dv01 is the change in present value for a 1 basis point change in the yield curve
		...
		if (riskType == "vega") {
			for (auto& kv : volShocks) {
				// to be added
				// string market_id = kv.first;
				// auto mkt = kv.second.getMarket();
				// double vol = mkt.getVolCurve(market_id)->getVol(Date()); // assuming
			}
